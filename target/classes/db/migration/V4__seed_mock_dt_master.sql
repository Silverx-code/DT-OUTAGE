-- Demo DT master data for local and staging environments.
-- The deterministic codes make this migration safe to run once through Flyway
-- and safe to re-apply manually without creating duplicates.
...
SELECT
    'DT-MOCK-' || lpad(n::text, 3, '0'),
    locations[((n - 1) % array_length(locations, 1)) + 1] || ' Transformer ' || (((n - 1) / array_length(locations, 1)) + 1),
    business_units[((n - 1) % array_length(business_units, 1)) + 1],
    undertakings[((n - 1) % array_length(undertakings, 1)) + 1],
    feeders[((n - 1) % array_length(feeders, 1)) + 1] || ' Feeder ' || (((n - 1) % 4) + 1),
